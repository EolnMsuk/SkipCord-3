...
from tools import BotState, BotConfig, get_discord_age, record_command_usage, record_command_usage_by_user, handle_errors, format_duration
from omegle import OmegleHandler
def format_departure_time(duration: timedelta) -> str:
    return format_duration(duration)
def create_message_chunks(entries: List[Any], title: str, process_entry: Callable[[Any], str], max_chunk_size: int=50, max_length: int=4000, as_embed: bool=False, embed_color: Optional[discord.Color]=None) -> Union[List[str], List[discord.Embed]]:
    ...
    for entry in entries:
        ...
        for processed in processed_list:
            if processed:
                entry_length = len(processed) + 1
                if current_length + entry_length > max_length and current_chunk or len(current_chunk) >= max_chunk_size:
                    if as_embed:
...
async def _button_callback_handler(interaction: discord.Interaction, command: str, helper: 'BotHelper') -> None:
    try:
        ...
        if user_id not in bot_config.ALLOWED_USERS:
            camera_required_commands = ['!skip', '!refresh', '!report', '!rules', '!mpauseplay', '!mskip', '!mshuffle', '!mclear']
            if command in camera_required_commands:
                ...
                if isinstance(user_member, discord.Member):
                    ...
                    is_in_vc_with_cam = bool(streaming_vc and user_member in streaming_vc.members and user_member.voice and user_member.voice.self_video)
            ...
            async with state.cooldown_lock:
                ...
                if time_since_last_cmd < 5.0:
                    ...
                    if not interaction.response.is_done():
                        ...
                        await interaction.followup.send(msg, ephemeral=True)
                    logger.warning(f'Blocked {interaction.user.name} from button {command} due to global cooldown.')
                    return
        ...
        try:
            ...
            logger.debug(f"Recorded button command '{command}' for user '{interaction.user.name}' in stats.")
        except Exception as e:
            logger.error(f'Failed to record button command usage in stats: {e}', exc_info=True)
        try:
            mock_ctx = type('obj', (object,), {'author': interaction.user, 'channel': interaction.channel, 'send': interaction.channel.send, 'bot': helper.bot, 'guild': interaction.guild, 'message': interaction.message, 'invoked_with': command.lstrip('!'), 'from_button': True})()
            if command == '!skip':
...
class HelpView(View):
    def __init__(self, helper: 'BotHelper'):
        ...
        cmds = [('⏸️', '👤', '!refresh', discord.ButtonStyle.danger), ('⏭️', '👤', '!skip', discord.ButtonStyle.success), ('ℹ️', '👤', '!rules', discord.ButtonStyle.primary), ('🚩', '👤', '!report', discord.ButtonStyle.secondary)]
        for e, l, c, s in cmds:
...
class QueueDropdown(discord.ui.Select):
    def __init__(self, bot, state, page_items, author):
        ...
        self.state = state
        self.author = author
        options = [discord.SelectOption(label=f"{i + 1}. {song_info.get('title', 'Unknown Title')}"[:100], value=str(i)) for i, song_info in page_items]
...
class MusicView(discord.ui.View):
    def __init__(self, helper: 'BotHelper'):
        super().__init__(timeout=None)
        btns = [('⏯️', '🎵', '!mpauseplay', discord.ButtonStyle.danger), ('⏭️', '🎵', '!mskip', discord.ButtonStyle.success), ('🔀', '🎵', '!mshuffle', discord.ButtonStyle.primary), ('❌', '🎵', '!mclear', discord.ButtonStyle.secondary)]
        for e, l, c, s in btns:
...
class BotHelper:
    ...
    async def _process_leave_batch(self):
        ...
        if count == 1:
            ...
            embed = discord.Embed(description=f"{member_data['mention']} **LEFT the SERVER**", color=discord.Color.red())
            ...
            embed = discord.Embed(title=f'🚪 Mass Departure Event', color=discord.Color.red())
            description_lines = []
    ...
    async def _log_timeout_in_state(self, member: discord.Member, duration_seconds: int, reason: str, moderator_name: str, moderator_id: Optional[int]=None):
        async with self.state.moderation_lock:
            self.state.active_timeouts[member.id] = {'timeout_end': time.time() + duration_seconds, 'reason': reason, 'timed_by': moderator_name, 'timed_by_id': moderator_id, 'start_timestamp': time.time()}
    ...
    async def handle_member_join(self, member: discord.Member) -> None:
        ...
        if chat_channel:
            embed = discord.Embed(description=f'{member.mention} **JOINED the SERVER**!', color=discord.Color.green())
            ...
            await chat_channel.send(embed=embed)
        async with self.state.moderation_lock:
            self.state.recent_joins.append((member.id, member.name, member.display_name, datetime.now(timezone.utc)))
    ...
    async def send_punishment_vc_notification(self, member: discord.Member, reason: str, moderator_name: str) -> None:
        ...
        if not chat_channel:
            return
        embed = discord.Embed(description=f'{member.mention} **was MOVED to the No Cam VC**', color=discord.Color.dark_orange())
    ...
    async def send_timeout_notification(self, member: discord.Member, moderator: discord.User, duration: int, reason: str=None) -> None:
        ...
        embed = discord.Embed(description=f'{member.mention} **was TIMED OUT**', color=discord.Color.orange())
        embed.set_author(name=f'{member.name}', icon_url=member.display_avatar.url)
        embed.set_thumbnail(url=member.display_avatar.url)
        try:
            user_obj = await self.bot.fetch_user(member.id)
    ...
    async def send_timeout_removal_notification(self, member: discord.Member, duration: int, reason: str='Expired Naturally') -> None:
        ...
        if not chat_channel:
            return
        duration_str = format_duration(duration)
        embed = discord.Embed(description=f'{member.mention} **TIMEOUT REMOVED**', color=discord.Color.orange())
        embed.set_author(name=f'{member.name}', icon_url=member.display_avatar.url)
        embed.set_thumbnail(url=member.display_avatar.url)
        try:
            user_obj = await self.bot.fetch_user(member.id)
    ...
    async def send_unban_notification(self, user: discord.User, moderator: discord.User) -> None:
        ...
        if chat_channel:
            ...
            await chat_channel.send(embed=embed)
            async with self.state.moderation_lock:
                self.state.recent_unbans.append((user.id, user.name, user.display_name, datetime.now(timezone.utc), moderator.name))
    ...
    async def handle_member_remove(self, member: discord.Member) -> None:
        ...
        async with self.state.moderation_lock:
            if member.id in self.state.recently_banned_ids:
                ...
                return
        try:
            async for entry in guild.audit_logs(limit=3, action=discord.AuditLogAction.kick, after=member.joined_at or datetime.now(timezone.utc) - timedelta(minutes=5)):
                time_difference = abs((entry.created_at - datetime.now(timezone.utc)).total_seconds())
                if entry.target and entry.target.id == member.id and (time_difference < 15):
                    reason = entry.reason or 'No reason provided'
        ...
        leave_data_for_notification = {'mention': member.mention, 'name': member.name, 'avatar_url': member.display_avatar.url, 'joined_at': member.joined_at, 'roles': role_string}
        async with self.state.moderation_lock:
    ...
    async def show_bans(self, ctx) -> None:
        ...
        embeds = create_message_chunks(entries=ban_entries, title=f'Banned Users (Total: {len(ban_entries)})', process_entry=process_ban, as_embed=True, embed_color=discord.Color.red())
        for embed in embeds:
    ...
    async def list_roles(self, ctx) -> None:
        ...
        for role in sorted(ctx.guild.roles, key=lambda r: r.position, reverse=True):
            if role.name != '@everyone' and role.members:
                ...
                embeds = create_message_chunks(entries=sorted_members, title=f'Role: {role.name}', process_entry=process_member, as_embed=True, embed_color=role.color or discord.Color.default())
    ...
    async def show_role_members(self, ctx, role: discord.Role) -> None:
        ...
        embeds = create_message_chunks(entries=sorted_members, title=f'Members in Role: {role.name} (Total: {len(role.members)})', process_entry=process_member, as_embed=True, embed_color=role.color or discord.Color.blue())
        for embed in embeds:
            await ctx.send(embed=embed)
    @handle_errors
    ...
    async def remove_timeouts(self, ctx) -> None:
        ...
        result_msg = []
        if removed:
            result_msg.append(f'**✅ Removed timeouts from:**\n- ' + '\n'.join(removed))
        if failed:
    ...
    async def show_timeouts(self, ctx) -> None:
        ...
        if disabled_user_ids:
            ...
            reports['🚫 Command Disabled Users'] = create_message_chunks(entries=processed_disabled, title='🚫 Command Disabled Users', process_entry=lambda x: x, as_embed=True, embed_color=discord.Color.dark_grey())
    ...
    async def create_times_report_embed(self) -> Optional[discord.Embed]:
        ...
        embed = discord.Embed(title='🏆 Top 10 VC Members', description='\n'.join(description_lines), color=discord.Color.gold())
        return embed
    @handle_errors
        ...
        if user_violations_data:
            ...
            async def process_violation(entry):
                ...
                if (member := guild.get_member(uid)):
                    ...
                    try:
                        user_display_str = f'`{(await self.bot.fetch_user(uid)).name}` (Left Server)'
                    except discord.NotFound:
    ...
    async def send_join_invites(self, ctx) -> None:
        ...
        for member in members_to_dm:
            ...
            try:
                ...
                logger.warning(f'Could not DM {member.name} (DMs are disabled or bot is blocked).')
            except Exception as e:
    ...
    async def clear_stats(self, ctx) -> None:
        confirm_msg = await ctx.send('⚠️ This will reset all statistics data (VC times, command usage, and violation counts). This does not affect moderation history.\nReact with ✅ to confirm or ❌ to cancel.')
        await confirm_msg.add_reaction('✅')
        await confirm_msg.add_reaction('❌')
        def check(reaction, user):
            return user == ctx.author and str(reaction.emoji) in ['✅', '❌'] and (reaction.message.id == confirm_msg.id)
    ...
    async def confirm_and_clear_music_queue(self, ctx_or_interaction: Union[commands.Context, discord.Interaction]) -> None:
        if isinstance(ctx_or_interaction, commands.Context):
            ...
            clear_react_func = lambda msg: interaction.edit_original_response(view=None)
            initial_message_content = f'Are you sure you want to clear the queue and stop playback?'
            message_obj_attr = 'message'
            if not interaction.response.is_done():
        ...
        confirm_msg = None
        if isinstance(ctx_or_interaction, discord.Interaction):
            confirm_msg = await interaction.followup.send(f'Are you sure you want to clear **{queue_length}** songs and stop playback?', view=confirm_view, ephemeral=True, wait=True)
        else:
            confirm_msg = await ctx.send(f'Are you sure you want to clear **{queue_length}** songs and stop playback?\nReact with ✅/❌.', view=confirm_view)
