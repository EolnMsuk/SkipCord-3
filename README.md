...
* `!enableomegle`: Enables Omegle features and launches the browser.
...
AUTO_VC_START = False                # Auto-starts stream when first user joins with cam
...
NORMALIZE_LOCAL_MUSIC = True         # Apply audio normalization to local files (requires FFmpeg)
...
### Troubleshooting
  ...
  * **Edge Won't Launch**: Double-check that the `EDGE_USER_DATA_DIR` path in `config.py` is absolutely correct and matches your system. Ensure Edge is closed before starting the bot.
