...
logger.add('ban.log', rotation='10 MB', compression='zip', enqueue=True, level='INFO', filter=lambda record: record['extra'].get('BAN_LOG', False), format='<green>{time:MM-DD-YYYY HH:mm:ss}</green> | <level>{message}</level>')
...
async def log_command_usage(state: 'BotState', ctx_or_interaction: Any, command_name: str) -> None:
    try:
        ...
        logger.info(f"COMMAND USED: '{command_name}' by '{user_nickname}' in #{safe_channel} at {human_time} [via {source}]")
                ...
                try:
                    if isinstance(reply_target, discord.Interaction):
                        if reply_target.response.is_done():
                            await reply_target.followup.send(error_message, ephemeral=True)
                        else:
                            await reply_target.response.send_message(error_message, ephemeral=True)
                    elif hasattr(reply_target, 'send'):
                        await reply_target.send(error_message)
                except Exception as send_e:
                    logger.error(f'Failed to send error message to context/interaction: {send_e}')
    return wrapper
...
ALLOWED_STATS_COMMANDS = {'!stats', '!skip', '!refresh', '!report', '!rules', '!about', '!info', '!whois', '!rtimeouts', '!roles', '!join', '!top', '!commands', '!admin', '!admins', '!owner', '!owners', '!timeouts', '!times', '!rhush', '!rsecret', '!hush', '!secret', '!modon', '!modoff', '!banned', '!bans', '!clearstats', '!start', '!pause', '!clearwhois', '!msearch', '!mclear', '!mshuffle', '!mpauseplay', '!mskip', '!nowplaying', '!np', '!queue', '!q', '!playlist', '!volume', '!mon', '!moff', '!help', '!music', '!purge', '!shutdown', '!disable', '!enable', '!disablenotifications', '!enablenotifications', '!ban', '!unbanall', '!display', '!move'}
...
class BotConfig:
    ...
    def from_config_module(config_module: Any) -> 'BotConfig':
        ...
        return BotConfig(GUILD_ID=getattr(config_module, 'GUILD_ID', None), COMMAND_CHANNEL_ID=getattr(config_module, 'COMMAND_CHANNEL_ID', None), CHAT_CHANNEL_ID=getattr(config_module, 'CHAT_CHANNEL_ID', None), STREAMING_VC_ID=getattr(config_module, 'STREAMING_VC_ID', None), PUNISHMENT_VC_ID=getattr(config_module, 'PUNISHMENT_VC_ID', None), OMEGLE_VIDEO_URL=getattr(config_module, 'OMEGLE_VIDEO_URL', None), EDGE_USER_DATA_DIR=getattr(config_module, 'EDGE_USER_DATA_DIR', None), SS_LOCATION=getattr(config_module, 'SS_LOCATION', 'screenshots'), LOG_GC=getattr(config_module, 'LOG_GC', None), ALT_VC_ID=getattr(config_module, 'ALT_VC_ID', []), ALLOWED_USERS=getattr(config_module, 'ALLOWED_USERS', set()), ADMIN_ROLE_NAME=getattr(config_module, 'ADMIN_ROLE_NAME', []), MOVE_ROLE_NAME=getattr(config_module, 'MOVE_ROLE_NAME', []), JOIN_INVITE_MESSAGE=getattr(config_module, 'JOIN_INVITE_MESSAGE', ''), ENABLE_GLOBAL_HOTKEY=getattr(config_module, 'ENABLE_GLOBAL_HOTKEY', False), GLOBAL_HOTKEY_COMBINATION=getattr(config_module, 'GLOBAL_HOTKEY_COMBINATION', 'alt+grave'), COMMAND_COOLDOWN=getattr(config_module, 'COMMAND_COOLDOWN', 5), RULES_MESSAGE=getattr(config_module, 'RULES_MESSAGE', default_rules), INFO_MESSAGES=getattr(config_module, 'INFO_MESSAGES', []), CAMERA_OFF_ALLOWED_TIME=getattr(config_module, 'CAMERA_OFF_ALLOWED_TIME', 30), TIMEOUT_DURATION_SECOND_VIOLATION=getattr(config_module, 'TIMEOUT_DURATION_SECOND_VIOLATION', 60), TIMEOUT_DURATION_THIRD_VIOLATION=getattr(config_module, 'TIMEOUT_DURATION_THIRD_VIOLATION', 300), STATS_EXCLUDED_USERS=getattr(config_module, 'STATS_EXCLUDED_USERS', set()), AUTO_STATS_CHAN=getattr(config_module, 'AUTO_STATS_CHAN', None), AUTO_STATS_HOUR_UTC=getattr(config_module, 'AUTO_STATS_HOUR_UTC', 0), AUTO_STATS_MINUTE_UTC=getattr(config_module, 'AUTO_STATS_MINUTE_UTC', 0), MEDIA_ONLY_CHANNEL_ID=getattr(config_module, 'MEDIA_ONLY_CHANNEL_ID', None), MOD_MEDIA=getattr(config_module, 'MOD_MEDIA', True), EDGE_DRIVER_PATH=getattr(config_module, 'EDGE_DRIVER_PATH', None), EMPTY_VC_PAUSE=getattr(config_module, 'EMPTY_VC_PAUSE', True), AUTO_VC_START=getattr(config_module, 'AUTO_VC_START', False), MUSIC_ENABLED=getattr(config_module, 'MUSIC_ENABLED', False), MUSIC_LOCATION=getattr(config_module, 'MUSIC_LOCATION', None), MUSIC_BOT_VOLUME=getattr(config_module, 'MUSIC_BOT_VOLUME', 0.2), MUSIC_MAX_VOLUME=getattr(config_module, 'MUSIC_MAX_VOLUME', 1.0), MUSIC_SUPPORTED_FORMATS=getattr(config_module, 'MUSIC_SUPPORTED_FORMATS', ('.mp3', '.flac', '.wav', '.ogg', '.m4a')), MUSIC_DEFAULT_ANNOUNCE_SONGS=getattr(config_module, 'MUSIC_DEFAULT_ANNOUNCE_SONGS', False), NORMALIZE_LOCAL_MUSIC=getattr(config_module, 'NORMALIZE_LOCAL_MUSIC', True), ENABLE_GLOBAL_MSKIP=getattr(config_module, 'ENABLE_GLOBAL_MSKIP', False), GLOBAL_HOTKEY_MSKIP=getattr(config_module, 'GLOBAL_HOTKEY_MSKIP', 'grave'), ENABLE_GLOBAL_MPAUSE=getattr(config_module, 'ENABLE_GLOBAL_MPAUSE', False), GLOBAL_HOTKEY_MPAUSE=getattr(config_module, 'GLOBAL_HOTKEY_MPAUSE', 'grave'), ENABLE_GLOBAL_MVOLUP=getattr(config_module, 'ENABLE_GLOBAL_MVOLUP', False), GLOBAL_HOTKEY_MVOLUP=getattr(config_module, 'GLOBAL_HOTKEY_MVOLUP', ']'), ENABLE_GLOBAL_MVOLDOWN=getattr(config_module, 'GLOBAL_HOTKEY_MVOLDOWN', False), GLOBAL_HOTKEY_MVOLDOWN=getattr(config_module, 'GLOBAL_HOTKEY_MVOLDOWN', '['))
...
async def build_role_update_embed(member: discord.Member, roles_gained: List[discord.Role], roles_lost: List[discord.Role]) -> discord.Embed:
    ...
    if member.joined_at:
        embed.add_field(name='Time in Server', value=get_discord_age(member.joined_at), inline=True)
    embed.add_field(name='User ID', value=str(member.id), inline=True)
    if roles_gained:
        embed.add_field(name='Roles Gained', value=' '.join([role.mention for role in roles_gained]), inline=False)
    if roles_lost:
        embed.add_field(name='Roles Lost', value=' '.join([role.mention for role in roles_lost]), inline=False)
    return embed
...
class BotState:
    ...
    def to_dict(self, guild: Optional[discord.Guild], active_vc_sessions_to_save: dict, current_time: float) -> dict:
        ...
        return {'analytics': self.analytics, 'omegle_enabled': self.omegle_enabled, 'relay_command_sent': self.relay_command_sent, 'users_received_rules': list(self.users_received_rules), 'user_violations': self.user_violations, 'active_timeouts': self.active_timeouts, 'notifications_enabled': self.notifications_enabled, 'move_command_cooldowns': self.move_command_cooldowns, 'recent_joins': [{'id': e[0], 'name': e[1], 'display_name': e[2], 'timestamp': e[3].isoformat()} for e in self.recent_joins], 'recent_leaves': [{'id': e[0], 'name': e[1], 'display_name': e[2], 'timestamp': e[3].isoformat(), 'roles': e[4]} for e in self.recent_leaves], 'recent_role_changes': [{'id': e[0], 'name': e[1], 'gained': e[2], 'lost': e[3], 'timestamp': e[4].isoformat()} for e in self.recent_role_changes], 'users_with_dms_disabled': list(self.users_with_dms_disabled), 'recent_bans': [{'id': e[0], 'name': e[1], 'display_name': e[2], 'timestamp': e[3].isoformat(), 'reason': e[4]} for e in self.recent_bans], 'recent_kicks': [{'id': e[0], 'name': e[1], 'display_name': e[2], 'timestamp': e[3].isoformat(), 'reason': e[4], 'moderator': e[5], 'roles': e[6]} for e in self.recent_kicks], 'recent_unbans': [{'id': e[0], 'name': e[1], 'display_name': e[2], 'timestamp': e[3].isoformat(), 'moderator': e[4]} for e in self.recent_unbans], 'recent_untimeouts': [{'id': e[0], 'name': e[1], 'display_name': e[2], 'timestamp': e[3].isoformat(), 'reason': e[4], 'moderator_name': e[5], 'moderator_id': e[6]} for e in self.recent_untimeouts], 'omegle_disabled_users': list(self.omegle_disabled_users), 'recent_kick_timestamps': {k: v.isoformat() for k, v in self.recent_kick_timestamps.items()}, 'vc_time_data': {str(user_id): data for user_id, data in vc_data_to_save.items()}, 'active_vc_sessions': {}, 'music_enabled': self.music_enabled, 'music_mode': self.music_mode, 'search_queue': [clean_song_dict(s) for s in self.search_queue], 'active_playlist': [clean_song_dict(s) for s in self.active_playlist], 'current_song': clean_song_dict(self.current_song), 'music_volume': self.music_volume, 'playlists': {p_name: [clean_song_dict(s) for s in songs] for p_name, songs in self.playlists.items()}, 'window_size': self.window_size, 'window_position': self.window_position, 'is_banned': self.is_banned, 'ban_message_id': self.ban_message_id, 'music_menu_message_id': self.music_menu_message_id, 'times_report_message_id': self.times_report_message_id, 'vc_moderation_active': self.vc_moderation_active, 'last_vc_connect_fail_time': self.last_vc_connect_fail_time}
    @classmethod
    def from_dict(cls, data: Dict[str, Any], config: BotConfig) -> 'BotState':
        ...
        state.move_command_cooldowns = {int(k): v for k, v in data.get('move_command_cooldowns', {}).items()}
    ...
    async def clean_old_entries(self) -> None:
        ...
        async with self.vc_lock, self.analytics_lock, self.moderation_lock, self.music_lock:
            ...
            self.vc_time_data = {user_id: {**data, 'sessions': [s for s in data.get('sessions', []) if s.get('end', 0) > seven_days_ago_ts]} for user_id, data in self.vc_time_data.items() if any((s.get('end', 0) > seven_days_ago_ts for s in data.get('sessions', [])))}
            ...
            list_specs = {'recent_joins': (3, 200), 'recent_leaves': (3, 200), 'recent_bans': (3, 200), 'recent_kicks': (3, 200), 'recent_unbans': (3, 200), 'recent_untimeouts': (3, 200), 'recent_role_changes': (4, 200)}
